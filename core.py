...
def generate_student_accounts():
    ...
    with open("./importi_cedole.csv", "r") as infile:
        reader = csv.reader(infile)
        next(reader)
        class_amounts = {row[1]: int((float(row[2]) * 1000000))
                         for row in reader}
        infile.close()

    with open("./lista_cedole.csv", "r") as std_list:
...
def create_contract_code():
    for addr, amnt in zip(addr_stud, funding_amount):
        with open("./upd_teal.teal", "r") as inf:
            with open('./stud_contracts/stud' + addr + '.teal', 'w') as outf:
                for line in inf:
...
def create_student_qr(addr, lsig):
    ...
    dict["lsig"]['sig'] = base64.urlsafe_b64encode(
        dict["lsig"]['sig']).decode()
    ...
    cedola.save("./stud_QR/qr" + addr + ".png")

    # KL save the data encoded in the QR to file for testing
    with open("./qr_data/data" + addr, "w") as f:
        f.write(data)
...
def generate_lsigs(stud_dict):
    for addr in addr_stud:
        ...
        try:
            with open("./stud_contracts/stud" + addr + ".teal", "r") as contract_f:
...
def send_min_bal(addresses):
    ...
    for i in range(len(addresses)):
        ...
        print("Final MASTER account balance: {} microAlgos".format(
        ...
        if addresses[0] == addr_stud[0]:
            ...
            print("Account balance: {} microAlgos".format(
                account_info.get('amount')) + "\n")
            print(
                ">>> END Send money from master to student {}      --------------------------------".format(addresses[i]))
        else:
            print("Final BOOKSHOP account for bookshop: "+addr_bookshop[i])
            print("Account balance: {} microAlgos".format(
                account_info.get('amount')) + "\n")
            print(
                ">>> END Send money from master to bookshop {}      --------------------------------".format(addresses[i]))
...
def initial_transfer():
    ...
    for addr in addr_stud:
        # 110k for 100k min balance requirement and 10k allowance for fees
        amount = int(addr_amounts[addr]) + 110000
        receiver = addr
        unsigned_txn = PaymentTxn(
            sender, params, receiver, amount, None, note, None, None)
        signed_txn = unsigned_txn.sign(master_account_private_key)
        txid = algod_client.send_transaction(signed_txn)
        print("Successfully sent transaction with txID: {}".format(txid))
        ...
        print("Final STUDENT account balance: {} microAlgos".format(
            account_info.get('amount')))
    return
...
def test_lsig_rebuild(addr_lsigs):
    ...
    for addr in addr_stud:
        ...
        img = cv2.imread("./stud_QR/qr" + addr + ".png")
        decoded = decode(img)
        data = decoded[0][0]
        data = data.decode()
...
def test_lsig_transactions(final_addr_lsigs):
    ...
    for addr in addr_stud:
        txn = PaymentTxn(addr, params, receiver,
                         amount, None, note, None, None)
        ...
        print("Final BOOKSHOP account balance: {} microAlgos".format(
            account_info.get('amount')))
...
def main():
    ...
    print("MASTER Account balance:  microAlgos  {}".format(
        account_info.get('amount')) + "\n")
    ...
    with open("./stud_data/stud_acc_pk_mn.csv", "w") as f:
        outf = csv.writer(f)
    ...
    with open("./stud_data/book_acc_pk_mn.csv", "w") as f:
        outf = csv.writer(f)
