...
bookshop_address = "XYQJHBCBAJJXJJ7SZPGUPU76CIFWLIPQGSAQDZPBUCC3ZFFKUPC3ZFBYP4"
...
def openScanner():
    ...
    with open('./qrResult', "w") as f:
        f.write(qr_result)
        f.close()

    # Reopen
    with open('./qrResult', "r") as f:
        data = f.read()
        print(data)
    ...
    hyperlink = Label(
        root, text="Click to view transaction history", fg="blue", cursor="hand2")
    ...
    button2 = Button(root, text="Enter",
                     command=lambda: saveInput(root, addr, fee, lsig))
...
def sendTransaction(addr, fee, lsig, root):
    ...
    try:
        ...
        err1.config(
            text="Network connection failed. Please check that sandbox is connected to algod client")
        err1.update()
    params.fee = fee
    ...
    try:
        ...
        upd2.config(text="Result confirmed in round: {}".format(
            confirmed_txn['confirmed-round']))
        upd2.update()
        print("Result confirmed in round: {}".format(
            confirmed_txn['confirmed-round']))
        upd3.config(text="Transaction information: {}".format(
        ...
        upd4.config(
            text="Transaction successfuly sent to bookshop OJW4R7C3SWQRO7KR4EI4H4KU7DVBW5EDDRMD63MGU2QJWHOVAHA6OGR2BU")
        upd4.update()

        account_info = algod_client.account_info(addr)
        bal = account_info.get("amount")
        upd5.config(
            text="REMAINING ACCOUNT BALANCE: {} ALGO ".format(bal / 1000000))
        upd5.update()

        print("\n\n\n END<<< Lsig transaction successfully sent by (simulated) EmFi Wallet")
        print(
            "        on behalf of student[0] to bookshop[0], without any additional signature\n\n\n")
...
# Open QR scanner
scanQrButton = Button(root, text="Scan Qr code", height=3,
                      width=384, command=lambda: scanQR(root))
