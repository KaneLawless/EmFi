...
council = "CJRJE3UTTZ6JLZVFY4E33AQMOUJT44RTCMUYP7EU3S6ENJ2QS27QR6SXWA"

# Bookshop accounts to retrieve transactions from
bookshop1 = "XYQJHBCBAJJXJJ7SZPGUPU76CIFWLIPQGSAQDZPBUCC3ZFFKUPC3ZFBYP4"
bookshop2 = "QEFFXYJDD3MU242P5LB5WX3FVHOVNNM3XQW74V75GJS3IPU4LHRCVDGJOI"

# Dates to retrieve transactions between
txn_start = "2022-10-10"
txn_end = "2022-10-12"
purestake_token = ""
...
def prepare_txn(txn_df):
    ...
    txn_df_reduced = txn_filtered[[
        "payment-transaction", "sender", "id", "round-time"]]

    # Get amount (microAlgos) and receiver info from transaction
    txn_df_reduced["amount"] = txn_df_reduced["payment-transaction"].apply(pd.Series)[
        "amount"]
    txn_df_reduced["receiver"] = txn_df_reduced["payment-transaction"].apply(pd.Series)[
        "receiver"]
    txn_df_reduced.drop(["payment-transaction"], axis=1, inplace=True)

    # Format round-time to show the date
    txn_df_reduced["date"] = txn_df_reduced["round-time"].map(
...
def generate_files(folder_path, cal, cal1):
    ...
    with open("./stud_data/stud_acc_pk_mn.csv", "r") as stud_data:
    ...
    with open("./lista_cedole.csv", "r") as f:
    ...
    cols = ['ANNO_SCOLASTICO', 'ID_MINORE', 'CLASSE', 'CODICE_SCUOLA',
            'NUMERO_TRANSAZIONE', 'DATA_TRANSAZIONE', 'IMPORTO_TRANSAZIONE']
    outfile = new_merged[cols]

    # Write DF to csv file
    outfile.to_csv(path_or_buf=folder_path.get() +
                   "/final_lista_cedole.csv", index=False)

    # Sort values for Riepologo Contabile file
    for_sorting = ["ANNO_SCOLASTICO", "receiver", "CODICE_SCUOLA",
                   "CLASSE", "IMPORTO_TRANSAZIONE", "NUMERO_TRANSAZIONE"]
    ...
    new_merged.to_csv(folder_path.get() +
                      "/riepologo_contabile.csv", index=False)
...
def main():
    ...
    generateFilesButton = Button(root, text="Generate Reports", height=3,
                                 width=384, command=lambda: generate_files(folder_path, cal, cal1))
